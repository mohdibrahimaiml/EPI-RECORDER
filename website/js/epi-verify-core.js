...
// ── window.verifyEPI: Public API for verify/index.html ──
// Returns {structure, manifest, integrity, hashChain, signature, hash, verification, mismatches, steps}
window.verifyEPI = async function(blob) {
  const result = await analyzeArtifactBlob(blob, { allowEmbeddedViewer: false });
  const manifest = result.manifest || {};
  const mismatches = result.mismatches || [];
  const steps = result.steps || [];
  const ver = result.verification || {};

  // Verify hash chain (prev_hash linkage)
  let hashChain = true;
  for (let i = 1; i < steps.length; i++) {
    if (steps[i].prev_hash) {
      const prevCanonical = canonicalJson(steps[i-1]);
      const prevHashHex = await sha256Hex(new TextEncoder().encode(prevCanonical));
      if (prevHashHex !== steps[i].prev_hash) {
        hashChain = false;
        break;
      }
    }
  ...
  return {
    ...
    hashChain: hashChain,
    signature: ver.signatureValid,
    hash: manifest.workflow_id || null,
    verification: ver,
