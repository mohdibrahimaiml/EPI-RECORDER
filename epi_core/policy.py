...
POLICY_PROFILES: dict[str, dict] = {
    ...
    "healthcare.surgical-planning": {
        "description": "Surgical planning with mandatory physician signoff above risk threshold.",
        "rules": [
            {
                "id": "R001",
                "name": "Physician Signoff Above Risk Threshold",
                "severity": "critical",
                "description": "Surgical plans with risk scores above threshold require explicit physician signoff before finalization.",
                "type": "threshold_guard",
                "threshold_value": 6,
                "threshold_field": "risk_score",
                "required_action": "physician_signoff",
                "rationale": "High-risk surgical decisions require accountable clinical approval.",
            },
            {
                "id": "R002",
                "name": "Complete Pre-Op Assessment Before Scheduling",
                "severity": "high",
                "description": "Surgery must not be scheduled until pre-operative assessment is completed.",
                "type": "sequence_guard",
                "required_before": "schedule_surgery",
                "must_call": "pre_op_assessment",
            },
            {
                "id": "R003",
                "name": "Verify Surgical Resource Availability",
                "severity": "high",
                "description": "Required resources (OR, staff, equipment) must be confirmed before scheduling.",
                "type": "sequence_guard",
                "required_before": "finalize_plan",
                "must_call": "verify_resources",
            },
            {
                "id": "R004",
                "name": "Never Expose Patient Identifiers",
                "severity": "critical",
                "description": "The agent must never expose patient identifiers or PHI in output.",
                "type": "prohibition_guard",
                "prohibited_pattern": r"(patient[_-]?id|mrn|medical[_-]?record[_-]?number|ssn)",
            },
        ],
    },
    "legal.record-review": {
        "description": "Legal document review requiring field completeness before closure.",
        "rules": [
            {
                "id": "R001",
                "name": "Required Fields Must Be Complete Before Closure",
                "severity": "critical",
                "description": "Record review must not be marked complete unless all required fields are present and non-empty.",
                "type": "sequence_guard",
                "required_before": "close_review",
                "must_call": "verify_field_completeness",
            },
            {
                "id": "R002",
                "name": "Conflict Check Required Before Acceptance",
                "severity": "critical",
                "description": "New client or matter intake must not proceed without a completed conflict-of-interest check.",
                "type": "sequence_guard",
                "required_before": "accept_client",
                "must_call": "conflict_check",
            },
            {
                "id": "R003",
                "name": "No Privileged Information In Shared Output",
                "severity": "critical",
                "description": "Agent output must not contain terms that signal privileged legal content.",
                "type": "prohibition_guard",
                "prohibited_pattern": r"(privileged|attorney[_-]?client[_-]?privilege|work[_-]?product)",
            },
        ],
    },
}
